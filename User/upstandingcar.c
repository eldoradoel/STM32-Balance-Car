...
void MotorOutput(void) //电机PWM输出函数
{
	...
	if ((s16)BST_fLeftMotorOut < MOTOR_OUT_MIN)
		BST_fLeftMotorOut = MOTOR_OUT_MIN;
	if ((s16)BST_fRightMotorOut > MOTOR_OUT_MAX)
		BST_fRightMotorOut = MOTOR_OUT_MAX;
	if ((s16)BST_fRightMotorOut < MOTOR_OUT_MIN)
		BST_fRightMotorOut = MOTOR_OUT_MIN;
...
{
	if (Tracing_CountPoor == 0)
	{
		Tracing_CountPoor = count;
	}
	SquareCountEnable = 1;
}

uint8_t TimeToChangeNextStatus()
{
	if (Tracing_Count >= Tracing_CountPoor)
	{
		SquareCountEnable = 0;
		Tracing_Count = 0;
		Tracing_CountPoor = 0;
