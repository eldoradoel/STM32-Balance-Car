  ...
  if (EXTI_GetITStatus(EXTI_Line1) != RESET)
  {
    ...
    while (count) //等待低电平
    {
      if (TIM_GetCounter(TIM1) >= 10) // 9*10us=90us
      {
        ...
        count = GPIO_ReadInputDataBit(ECHO_PORT, ECHO_PIN); //检测PB1是否高电平 高 则count为1 低 则count为0
    }
...
{
  GPIO_SetBits(TRIG_PORT, TRIG_PIN);   //送>10US的高电平?T  本指令与：GPIO_SetBits(PB,GPIO_Pin_0) 等同
  delay_us(12);                        //延时11US
